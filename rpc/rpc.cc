/*
 ...
 ensure that there are no outstanding references to a to-be-deleted object. For
 rpcc, multiple client threads might be invoking the rpcc::call() functions and
 thus holding multiple references to the underlying connection object. For rpcs,
...
int rpcc::call1(unsigned int proc, marshall &req, unmarshall &rep, TO to) {
  ...
  {
    ScopedLock ml(&m_); // no locking of ca.m because no one but this thread
                        // changes ca.xid
    calls_.erase(ca.xid);
  ...
  jsl_log(JSL_DBG_2,
          "rpcc::call1 %u wait over for req proc %x xid %u %s:%d done? %d ret "
          "%d \n",
...
/**
 * clt_nonce: if not 0, this is a client that requires at-most-once semantics,
 * and clt_nonce is the client id xid: current transaction id xid_rep: latest
 * transaction id by client, aka window start index (this transaction had been
 * finished) b: buffer to be returned sz: size of the buffer
 */
rpcs::rpcstate_t rpcs::checkduplicate_and_update(unsigned int clt_nonce,
  ...
  // should be handled by client, but we could check for robustness
  if (xid <= xid_rep) {
    return FORGOTTEN;
  }

  // all xid<=xid_rep have finished, update our window first
  if (reply_window_.find(clt_nonce) == reply_window_.end()) {
    assert(reply_window_[clt_nonce].size() == 0); // create
    jsl_log(JSL_DBG_2,
            "rpcs::checkduplicate_and_update: new client %u xid %d chan %d, "
            "total clients %d\n",
            clt_nonce, xid, 0, (int)reply_window_.size());
  }
  std::list<reply_t> &l = reply_window_[clt_nonce];
  // 1. remove old, confirmed entries
  for (auto it = l.begin(); it != l.end();) {
    if ((*it).xid < xid_rep) {
      ...
      // TODO: if our window list is sorted, then we could just break here
      //       but traverse all the element is **always** correct
      it++;
    }
  }

  // 3. should reuse the old one or create a new item
  std::list<reply_t> &l2 = reply_window_.at(clt_nonce);
  auto it2 = std::find_if(l2.begin(), l2.end(),
                          [xid](reply_t &reply) { return reply.xid == xid; });

  // 2. check if we have the ongoing request or done request)
  if (it2 != l2.end()) {
    if ((*it2).cb_present) {
      *b = (*it2).buf;
      *sz = (*it2).sz;
    ...
    l.emplace_back(reply_t(xid));
    return NEW;
  }
}

// now the reply is valid
void rpcs::add_reply(unsigned int clt_nonce, unsigned int xid, char *b,
                     int sz) {
  ScopedLock rwl(&reply_window_m_);

  auto &l = reply_window_.at(clt_nonce);
