...
void *client4(void *xx) {
  ...
  // if the server cannnot handle at-most-once logic properly
  // the client will continue to retry the RPC and the server
  // so the resources will not be freed.
  ...
  if (!(final - initial < THRES)) {
    ...
    printf("final: %d, initial: %d, thres: %f\n", (int) final, (int)initial,
           THRES);
  }
...
void failure_test() {
  ...
  printf("   -- create new client and try to bind to failed server .. failed "
         "ok\n");
  ...
  printf("   -- delete existing rpc client, create replacement rpc client .. "
         "ok\n");
  ...
  clients[0] = client = new rpcc(dst);
  assert(client->bind() >= 0);
  printf("   -- delete existing rpc client and server, create replacements.. "
         "ok\n");
