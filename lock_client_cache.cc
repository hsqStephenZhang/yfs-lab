    ...
    case NONE:
...

      {
        ...
        lock_protocol::status ret =
            cl->call(lock_protocol::acquire, lid, id, r);
...
lock_protocol::status lock_client_cache::release(lock_protocol::lockid_t lid) {
  ...
  if (it == locks.end()) {
    pthread_mutex_unlock(&m);
    std::cout << "lock_client_cache::release: lock " << lid
              << " not found for client " << id << ", thread id: " << pthread_self()
              << std::endl;
    return lock_protocol::NOENT;
  }

  lock_entry *entry = it->second;
  std::cout << "lock_client_cache::release: releasing lock " << lid
            << " for client " << id << ", thread id: " << pthread_self() << ", revoked: " << entry->revoked
            << std::endl;
  ...
  if (entry->revoked) {
    ...
    std::cout << "lock_client_cache::release: lock " << lid
              << " released by client " << id << ", ret: " << ret << std::endl;
...
lock_client_cache::revoke_handler(lock_protocol::lockid_t lid, int &) {
  ...
  lock_entry *entry = get_lock_entry(lid);
  entry->revoked = true;
  std::cout << "lock_client_cache::revoke_handler: lock " << lid
            << " revoked for client " << id << ", state: " << entry->state << std::endl;
  ...
  if (entry->state == FREE) {
    ...
    int r;
    int res = cl->call(lock_protocol::release, lid, id, r);
    std::cout << "lock_client_cache::revoke_handler: lock " << lid
...
lock_client_cache::retry_handler(lock_protocol::lockid_t lid, int &) {
  ...
  std::cout << "lock_client_cache::retry_handler: retry for lock " << lid
            << " requested for client " << id << std::endl;
